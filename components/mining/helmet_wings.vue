  ...
  <div class="mining_pool">
    <div class="deposit" v-if="TradeType == 'STAKE' || TradeType == 'ALL'">
      ...
      <div class="button">
        ...
        <section>
          ...
          <a
            href="https://exchange.pancakeswap.finance/?_gl=1*d1kv5p*_ga*MTU5MDI5ODU1LjE2MTE5MzU1ODc.*_ga_334KNG3DMQ*MTYxMjg1NDcwNy4xOC4xLjE2MTI4NTQ4MzUuMA..#/add/0x34508EA9ec327ff3b98A2F10eEDc2950875bf026/0x948d2a81086A075b3130BAc19e4c6DEe1D2E3fE8"
            target="_blank"
            >From <i class="pancake"></i>Get HELMET-hWINGS LPT</a
          >
        </section>
      </div>
      ...
      <div class="addToken">
        <p @click="addTokenFn('HWINGS')">Add hWINGS to MetaMask</p>
        <i></i>
      </div>
    </div>
    <i></i>
    <div class="withdraw" v-if="TradeType == 'CLAIM' || TradeType == 'ALL'">
      ...
      <div class="button">
        ...
        <p>
          <span>WINGS {{ $t("Table.HELMETRewards") }}：</span>
          <span>
            <span>
              <countTo
                v-if="isLogin"
      ...
      <div class="ContractAddress">
        <span>WINGS {{ $t("Table.ContractAddress") }}</span>
        <p>
          ...
          <i
            ...
            id="copy_default"
            @click="
              copyAdress($event, '0x0487b824c8261462f88940f97053e65bdb498446')
            "
          ></i>
        </p>
      </div>
      <div class="addToken">
        <p @click="addTokenFn('WINGS')">Add WINGS to MetaMask</p>
        <i></i>
      </div>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      list: {
        name: "HELMET-xWINGS LP",
        dueDate: "2021/06/26 00:00",
        DownTime: {
          day: "00",
          hour: "00",
          minute: "00",
  ...
  mounted() {
    ...
    this.$bus.$on("DEPOSITE_LOADING_HELMETWINGS", (data) => {
      this.stakeLoading = data.status;
      this.DepositeNum = "";
    });
    this.$bus.$on("CLAIM_LOADING_HELMETWINGS", (data) => {
      this.claimLoading = false;
    });
    this.$bus.$on("EXIT_LOADING_HELMETWINGS", (data) => {
      this.exitLoading = false;
    });
    this.$bus.$on("RELOAD_DATA_HELMETWINGS", () => {
      this.getBalance();
    });
  ...
  methods: {
    ...
    async getBalance() {
      let helmetType = "HELMETWINGS_LPT";
      let type = "HELMETWINGS";
      // 可抵押数量
