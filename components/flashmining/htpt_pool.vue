...
export default {
  ...
  mounted() {
    ...
    this.$bus.$on("DEPOSITE_LOADING_HTPTPOOL", (data) => {
      this.stakeLoading = data.status;
      this.DepositeNum = "";
    });
    this.$bus.$on("CLAIM_LOADING_HTPTPOOL", (data) => {
      this.claimLoading = false;
    });
    this.$bus.$on("EXIT_LOADING_HTPTPOOL", (data) => {
      this.exitLoading = false;
    });
    this.$bus.$on("RELOAD_DATA_HTPTPOOL", () => {
      this.getBalance();
    });
